...
import tn.neuron.ardhi.services.UserAndDiag.PreventionTaskService;
...
public class AdminPreventionTasksController implements Initializable {

    @FXML
    private TableView<PreventionTask> tableTasks;
    @FXML
    private TableColumn<PreventionTask, Integer> colId;
    @FXML
    private TableColumn<PreventionTask, Integer> colPlanId;
    @FXML
    private TableColumn<PreventionTask, Integer> colDay;
    @FXML
    private TableColumn<PreventionTask, String> colDescription;
    @FXML
    private TableColumn<PreventionTask, String> colStatus;
    ...
    private List<PreventionTask> allTasks = new ArrayList<>();

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        try {
            colId.setCellValueFactory(new PropertyValueFactory<>("id"));
            colPlanId.setCellValueFactory(new PropertyValueFactory<>("preventionPlanId"));
            ...
            tableTasks.getSelectionModel().selectedItemProperty().addListener((obs, old, sel) -> {
                if (sel != null) {
                    ...
                    tfPlanId.setText(String.valueOf(sel.getPreventionPlanId()));
    ...
    void ajouterTask(ActionEvent event) {
        ...
        PreventionTask task = new PreventionTask(planId, dayOffset, desc.trim());
    ...
    void modifierTask(ActionEvent event) {
        PreventionTask selected = tableTasks.getSelectionModel().getSelectedItem();
        if (selected == null)
            return;
        ...
        selected.setPreventionPlanId(planId);
